...
for ((i=1; i<=$1; i++)); do
    echo "Iteration $i"
    echo "-------------------------"
    result=$(copilot --allow-all --resume --model "claude-sonnet-4.5" -p "@plans/prd.test.json @progress.test.txt \
...
2. Refactor the code as needed to ensure it is clean and maintainable. \
